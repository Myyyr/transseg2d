...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0., gt_num=1):
        ...
        self.global_token = torch.nn.Parameter(torch.zeros(gt_num,self.dim))
    ...
    def forward(self, x, mask=None):
        ...
        # elif self.count_forward==self.max_forwad:
        #     self.global_token += (self.mean_emb/self.n_mean_emb)[None,:]
        #     self.global_token.requires_grad = True

        if self.count_forward==self.max_forwad:
            self.global_token = torch.Parameter(repeat(self.mean_emb, 'c -> g c', g=self.gt_num))
