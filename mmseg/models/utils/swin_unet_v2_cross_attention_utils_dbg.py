...
class CrossAttentionBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, skip_connection_resolution, num_heads, window_size=7, shift_size=0,
        ...
        self.residual_patch_expand = residual_patch_expand # patch expand or bilinear
    ...
    def forward(self, x, x_downsampled, padwh):
        ...
        if self.residual_patch_expand:
            shortcut, Wh, Ww = self.expand(x, H, W, padwh) # shortcut : skip
        else:
...
class BasicLayer_up_Xattn(nn.Module):
    ...
    def __init__(self, dim, input_resolution, skip_connection_resolution, depth, num_heads, window_size,
        ...
        self.use_cross_attention = use_cross_attention # sert à rien
...
class SwinTransformerCrossAttentionSys(nn.Module):
    r""" Swin Transformer
    ...
    def __init__(self, pretrain_img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        for i_layer in range(self.num_layers-1):
            ...
            layer_up = BasicLayer_up_Xattn(dim=int(embed_dim * 2 ** (self.num_layers-1-i_layer)),
                                           input_resolution=(patches_resolution[0] // (2 ** (self.num_layers-1-i_layer)),
                                                             patches_resolution[1] // (2 ** (self.num_layers-1-i_layer))),
                                           skip_connection_resolution=((patches_resolution[0] // (2 ** (self.num_layers-1-i_layer)))*2,
                                                                       (patches_resolution[1] // (2 ** (self.num_layers-1-i_layer)))*2),
                                           depth=depths[(self.num_layers-1-i_layer-1)],
                                           num_heads=num_heads[(self.num_layers-1-i_layer-1)],
                                           window_size=window_size,
                                           mlp_ratio=self.mlp_ratio,
                                           qkv_bias=qkv_bias, qk_scale=qk_scale,
                                           drop=drop_rate, attn_drop=attn_drop_rate,
    ...
    def forward_features(self, x):
        ...
        for inx, layer in enumerate(self.layers):
            ...
            # print()
            # print('Enter Layer_down inx: ', inx)
            # print(x.shape)
            ...
            # print('Leave Layer_down inx: ', inx)
            # print(x.shape)
    ...
    def forward_up_features(self, x, x_downsample, x_downsample_resolutions, Wh, Ww, padswh):
        ...
        for inx, layer_up in enumerate(self.layers_up):
            ...
            # print()
            # print('Enter Layer_up inx: ', inx)
            # print(x.shape)
            # print(x_downsample[2-inx].shape)

            Wh_d, Ww_d = x_downsample_resolutions[2-inx]
            x, Wh, Ww = layer_up(x, x_downsample[2-inx], Wh, Ww, Wh_d, Ww_d, padwh)

            # print('Leave Layer_up inx: ', inx)
            # print(x.shape)
...
class SwinTransformerCrossAttentionUpsampleSys(nn.Module):
    r""" Swin Transformer
        A PyTorch impl of : `Swin Transformer: Hierarchical Vision Transformer using Shifted Windows`  -
          https://arxiv.org/pdf/2103.14030
    Args:
