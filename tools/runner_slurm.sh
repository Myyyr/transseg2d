#!/bin/bash
#SBATCH --job-name=zsg10tcitc     # job name
...
#SBATCH --output=logs/zsg10tcitc%j.out # output file name
#SBATCH --error=logs/zsg10tcitc%j.err  # error file name
#SBATCH -C v100-32g
...
srun /gpfslocalsup/pub/idrtools/bind_gpu.sh python -u tools/train.py $CONFIG --resume-from=$RESUME --launcher="slurm" ${@:3} --seed 0 --deterministic ${@:3} --no-validate
